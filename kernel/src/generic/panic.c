...
/// reason and terminate the execution by going to the bootloader
void panic_handler(const char* file_name, u32 line_number, const char* reason) {
    ...
    // it when it starts. To prevent that, the panic will copy the boot
    // signature into the boot signature area
