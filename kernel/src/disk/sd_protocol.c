...
/*
 * Asks the card to send its CID number. This is manatory. After a
 * successful CMD2 the card will enter identification state. The
 * functions returns the 16-byte CID register
 */
...
/*
 * Asks the card to publish a new relcative card address (RCA). This
 * will automatically update the slot 1 structure because only one
 * card is supported. Therefore the first published address will
 * allways be valid
 */
...
/*
 * Sends the supplied voltage to the card. It will echo back the
 * voltage and the check pattern if it supports the voltage. This
 * functions returns `1` if the supplied voltage is supported. If
 * not it returns `0`. This means one of two things; either the
 * card does not support the voltage, or the card is Ver1.X SD card.
 * In the first case the ACMD41 should be called with HCS equal zero.
 * If HCS is zero Ver2.00 or later SD card will not respond to this.
 */
static u8 sd_exec_cmd_8(u8 vhs) {
    ...
    /*
     * The SD card should echo the checkpattern and operating voltage
     * if supported. If SD v1.0 the card will not responde. If SD v2.0
     * and voltage mismatch the card is unusable.
     */
...
/*
 * Switch to high speed SDR-25. NOTE the user must check the SD
 * version before using this command. This command is NOT supporte
 * by version 1.0 and 1.01
 */
...
/*
 * Tells the card the next command to be sent is an application
 * specific command
 */
